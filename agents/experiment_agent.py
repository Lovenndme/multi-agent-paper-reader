...
def run_experiment_agent(paper_text: str) -> ExperimentOutput:
    ...
    return invoke_structured_with_retry(ExperimentOutput, build_experiment_messages(paper_text))


def stream_experiment_agent(paper_text: str, on_token: Callable[[str], None]) -> ExperimentOutput:
