...
下一次用户问题
        ...
        └─ 本地 MiniLM 向量检索
                  ├─ 完整查询：严格阈值召回
                  └─ 复合问题：最多 6 个子句分别 Top-1
                               │
                        去重并保留最多 3 条
                               │
                 论文证据后、最近消息前写入 Prompt
```

完整查询继续使用默认 0.48 相似度阈值；只有包含多个问号子句的复合问题才对每个
子句使用降低 0.15 的阈值，避免一次 pooled embedding 稀释多个意图。最终结果仍按
记忆 ID 去重并受 Top-3 限制。Prompt 装配时，命中的长期记忆使用
`min(5000, max(800, token_budget / 12))` 的动态上限，防止超长近期历史将其完全
挤出上下文。
