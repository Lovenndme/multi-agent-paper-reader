...
- 设置 API：`GET /api/settings` 在不暴露凭据的前提下返回项目版本和当前模型；`POST /api/settings/api-key` 会先验证 GLM Key，再保存到本机
...
首次使用时，可直接打开右上角的 **Settings**，粘贴智谱 GLM API Key 后点击“验证并保存”。后端会先发起一次最小真实请求验证 Key，成功后才写入本机 `.env` 文件，且不会把 Key 回传给浏览器。也可以将 `.env.example` 复制为 `.env`，再手动填写 `GLM_API_KEY`。项目默认连接智谱 GLM 的 `https://open.bigmodel.cn/api/paas/v4`，使用 `glm-5.2`。Agent 生成温度由 `LLM_TEMPERATURE` 控制；基于证据的论文追问使用独立的低温配置 `CHAT_TEMPERATURE`，默认值为 `0.25`。`CHAT_INPUT_TOKEN_BUDGET` 用于设置证据、近期对话和长期记忆共享的保守动态输入预算，默认值为 `48000`。
