...
运行环境必须有 Python 3.10 或更高版本。Git/源码工作区还需要带 npm 的
Node.js 18 或更高版本，用于重新构建前端。正式 Release ZIP 已包含验证过的前端；
只要该构建完整且版本匹配，即使没有 Node.js 也可完成安装。开发者建议使用 Git 克隆。
...
先关闭正在运行的旧服务，再在项目根目录执行更新脚本。Windows PowerShell：
...
如果项目来自 Git 克隆，脚本只有在所有已跟踪文件和暂存区均无修改时，才会执行
`git pull --ff-only`。脚本不会删除未跟踪文件，也不会覆盖本机 `.env` 和
`.paper-reader/` 数据。随后它会安装锁定的 Python 依赖、执行 `npm ci`、重新构建生产前端，
并将 `frontend-prototype/dist/build-meta.json` 与后端 `PROJECT_VERSION` 做强制一致性校验。
这是一项硬门禁：元数据缺失、损坏或版本不一致时，脚本会以非零状态退出，不能视为更新成功。
更新完成后必须使用脚本最后显示的命令重启服务；旧进程不会自动加载新代码。

如果原安装来自压缩包，其中没有 Git 元数据，更新脚本会有意跳过源码下载。请从 GitHub Releases
下载最新的 `Paper-Reader-<版本>.zip` 及对应 `.zip.sha256`，核对校验值后解压。把原目录中的
`.env` 和 `.paper-reader/` 复制到新项目目录完成数据迁移（旧目录先保留作备份），再在新目录执行
更新脚本。正式发行包已经包含版本匹配的生产前端，脚本校验后会直接复用，
无需 Node.js。不要把旧的 `frontend-prototype/dist` 复制进去。
