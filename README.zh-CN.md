...
- **分层检索 + 自适应 Agentic RAG**：默认采用 token 子块、父块回并、BM25/Dense RRF 与本地 Cross-Encoder 重排，仅在 Supervisor 定向修复、复杂数值/图表/跨章节问题等场景调用只读论文工具。
...
- **本地历史与记忆**：论文、分析、会话和长期记忆保存在本机 `.paper-reader/`；
  复合问题采用有界子句召回，并为命中的长期记忆保留独立 Prompt 预算。
